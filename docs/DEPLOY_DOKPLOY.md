...
Compose는 읽기 전용 루트 파일시스템, 모든 capability 제거, `no-new-privileges`, PID·CPU·메모리 제한, 실행 불가 임시 디렉터리, 로그 회전을 적용합니다. 자동 업데이트는 컨테이너의 불변성과 읽기 전용 파일시스템을 유지하기 위해 강제로 비활성화됩니다.
...
healthcheck가 `healthy`가 되고 Telegram polling이 시작되는지 확인합니다. 재배포 후에도 기존 SQLite 파일과 캐시 기록이 유지되는지 확인합니다.
